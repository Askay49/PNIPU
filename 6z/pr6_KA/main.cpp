...
void TName::Draw(int icolor) //функция отрисовки букв
{
	...
	GrEllipseArc(x, y + hn / 2, wn / 2, hn, 0, 1800, GR_ARC_STYLE_OPEN, icolor); // A
	GrLine(x - wn / 2, y, x + wn / 2, y, icolor);

	GrLine(x + 50, y - hn / 2, x + 50, y + hn / 2, icolor);
	GrEllipseArc(x + wn / 2 + 50, y, wn / 2, hn / 2, 900, 2700, GR_ARC_STYLE_OPEN, icolor); // K
}
...
main()
{
	...
	for (i = 0; i < N; i++) //цикл создания случайных фигур
	{
		...
		RandSel = rand() % 5; //обновляем случайное число
	}
	GrKeyType key;
	do
	{
