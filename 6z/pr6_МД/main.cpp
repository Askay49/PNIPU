...
{
	GrEllipseArc(x, y, wn / 2, hn, 0, 1800, GR_ARC_STYLE_OPEN, icolor); //буква М
	GrEllipseArc(x + wn, y, wn / 2, hn, 0, 1800, GR_ARC_STYLE_OPEN, icolor);

	GrLine(x + 100, y, x + wn * 2 + 100, y, icolor); //буква Д
	GrEllipseArc(x + wn + 100, y, wn / 2, hn, 0, 1800, GR_ARC_STYLE_OPEN, icolor);
}
...
main()
{
	...
	for (i = 0; i < N; i++) //цикл заполняющий фигурами массив
	{
		...
		obj[i]->Show();
		RandSel = rand() % 5; //обновляем переменную, чтобы новые фигуры были
	}
	GrKeyType key;
	do
	{
