...
int main()
{
	...
	for (int c = 0; c < size * 3; c++)
	{
		...
		for (int i = 0; i < size - 1; i++)
		{
			...
			if (*(matrix + i * size + (j + 1)) > *(matrix + (i + 1) * size + j))
			{
				k = *(matrix + i * size + (j + 1));
				*(matrix + i * size + (j + 1)) = *(matrix + (i + 1) * size + j);
				*(matrix + (i + 1) * size + j) = k;
			}
			if (*(matrix + (i + 1) * size + j) > *(matrix + (i + 1) * size + (j + 1)))
			{
				k = *(matrix + (i + 1) * size + j);
				*(matrix + (i + 1) * size + j) = *(matrix + (i + 1) * size + (j + 1));
	...
	printf("Минимальное число среди несортированных: %d\n\n", min);
